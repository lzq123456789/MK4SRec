...
class CoSeRecTrainer(Trainer):
    ...
    def _one_pair_contrastive_learning(self, inputs,  intent_ids=None):
        """
        ...
        if self.args.seq_representation_instancecl_type == "mean":
            weights = torch.rand(cl_sequence_output.size(0), cl_sequence_output.size(1),device='cuda')
            cl_sequence_output = torch.mean(cl_sequence_output* weights.unsqueeze(2), dim=1, keepdim=False)
        ...
        if mode == 'train':
            ...
            if self.args.contrast_type in ["IntentCL", "Hybrid"] and epoch >= self.args.warm_up_epoches:
                ...
                for i, (rec_batch, _, _) in rec_cf_data_iter:
                    ...
                    # average sum
                    if self.args.seq_representation_type == "mean":
                        weights = torch.rand(sequence_output.size(0), sequence_output.size(1),device='cuda')
                        sequence_output = torch.mean(sequence_output* weights.unsqueeze(2), dim=1, keepdim=False)
                    ...
                    if self.args.contrast_type == "InstanceCL":
                        ...
                        cl_losses.append(self.args.cf_weight * cl_loss)
                         ...
                         if epoch >= self.args.warm_up_epoches:
                             if self.args.seq_representation_type == "mean":
                                 weights = torch.rand(sequence_output.size(0), sequence_output.size(1), device='cuda')
                                 sequence_output = torch.mean(sequence_output * weights.unsqueeze(2), dim=1,
                                                              keepdim=False)
                             sequence_output = sequence_output.view(sequence_output.shape[0], -1)
                             sequence_output = sequence_output.detach().cpu().numpy()
