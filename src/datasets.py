...
class RecWithContrastiveLearningDataset(Dataset):
    def __init__(self, args, user_seq, time_seq, not_aug_users=None, data_type='train',
                 similarity_model_type='offline', total_train_users=0, no_augment=True):
        self.args = args
        ...
        if not self.no_augment:
            self.augmentations = {
                'crop': Crop(args.crop_mode, args.crop_rate),
                'mask': Mask(self.similarity_model, args.mask_mode, args.mask_rate),
                'reorder': Reorder(args.reorder_mode, args.reorder_rate),
                'substitute': Substitute(self.similarity_model, args.substitute_mode, args.substitute_rate),
            ...
            if self.args.base_augment_type not in self.augmentations:
                raise ValueError(f"augmentation type: '{self.args.base_augment_type}' is invalid")
            ...
            self.base_transform = lambda x, t: x
